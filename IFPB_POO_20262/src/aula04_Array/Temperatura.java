package aula04_Array;

import java.util.Scanner;

public class Temperatura {
	
	public static void main(String[] args) {
		/*
		 * Escreva um programa que peça para o usuário
		 * digitar n temperaturas e em seguida
		 * exibir a temperatura média do dia
		 */
		
		Scanner scanner = new Scanner(System.in);
		
		System.out.print("Digite a quantidade de registros de temperatura: ");
		int quantidade = scanner.nextInt();
		
		double [] temperaturas = new double [quantidade];
		
		double soma = 0;
		for(int i = 0; i < temperaturas.length; i++) {
			System.out.print("Temperatura["+i+"] = ");
			temperaturas[i] = scanner.nextDouble();
			soma = soma + temperaturas[i];
		}
		
		double temp_media = soma / quantidade;
		System.out.println("Temperatura média = "+temp_media);
	}

}
